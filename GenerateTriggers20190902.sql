...
/****** Object:  StoredProcedure [dbo].[GenerateTriggers]    Script Date: 9/2/2019 12:39:13 PM ******/
SET ANSI_NULLS ON
GO
...
IF EXISTS( SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '' + 
	...
	BEGIN
		SELECT @SQLTrigger = @SQLTrigger  + CHAR(10) + ' INSERT INTO ' + @QuotedSchemaName + '.' + @QuotedAuditTableName + CHAR(10) +  
		 ...
		 'SELECT ' + @ColList + ',''New'',''Insert'',CreatedBy,NULL,getdate(),''''  FROM INSERTED'
	END
	ELSE
	BEGIN
		SELECT @SQLTrigger = @SQLTrigger  + CHAR(10) + ' INSERT INTO ' + @QuotedSchemaName + '.' + @QuotedAuditTableName + CHAR(10) +  
		 '(' + @InsertColList + ')' + CHAR(10) +  
		 'SELECT ' + @ColList + ',''New'',''Insert'',SUSER_SNAME(),NULL,getdate(),''''  FROM INSERTED'
	END
	...
	BEGIN
		SELECT @SQLTrigger = @SQLTrigger + CHAR(10) + '  
		   ...
		   BEGIN			
				INSERT INTO ' + @QuotedSchemaName + '.' + @QuotedAuditTableName + CHAR(10) +  
				 '(' + @InsertColList + ')' + CHAR(10) +  
				 'SELECT ' + @ColList +',''New'',''Update'',UpdatedBy,NULL,getdate(),@UpdatedCols  FROM INSERTED    
   
				  INSERT INTO ' + @QuotedSchemaName + '.' + @QuotedAuditTableName + CHAR(10) +  
				 '(' + @InsertColList + ')' + CHAR(10) +  
				 'SELECT ' + @ColList +',''Old'',''Update'',UpdatedBy,NULL,getdate(),@UpdatedCols  FROM DELETED 
		   END'
	END
	ELSE
	BEGIN
